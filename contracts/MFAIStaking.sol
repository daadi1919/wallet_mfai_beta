...
contract MFAIStaking is ReentrancyGuard, Ownable {
    ...
    constructor(address _mfaiToken) {
        mfaiToken = IERC20(_mfaiToken);
    }
    
    function stake(uint256 _amount) external nonReentrant {
        require(_amount > 0, "Cannot stake 0");
        require(mfaiToken.transferFrom(msg.sender, address(this), _amount), "Transfer failed");
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(mfaiToken.transfer(msg.sender, _amount), "Transfer failed");
